<template>
    <!-- /all tasks of a bid -->
    <div>
        <div class="col-md-4" v-for="jobTask in bid.job_tasks" v-if="jobTask !== null" v-bind:key="jobTask.id"
             :id="'task-' + jobTask.id">
            <div class="panel">
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-xs-6 form-group">
                            ...
                            <input type="tel" class="form-control form-control-text" v-if="showTaskPriceInput()"
                        ...
                        <div class="col-xs-12 form-group" v-if="isContractor">
                            ...
                            <input type="date" class="form-control form-control-date" v-if="showTaskStartDate()"
                        ...
                        <section class="col-xs-12" style="margin-bottom: 8px;">
                            <a target="_blank"
                               :href="'https://www.google.com/maps/search/?api=1&query=' + location(jobTask)">
                    ...
                    <div class="row">
                                ...
                                <div class="col-xs-12 hidden" :id="'task-options-' + jobTask.id" :key="2">
                                    <div class="col-xs-4">
                                        ...
                                        <label v-if="showStripeToggle(jobTask)" class="switch">
                                            <input :id="'toggle-stripe-' + jobTask.id" type="checkbox"
                                                   v-model="jobTask.stripe" @click="toggleStripePaymentOption(jobTask)">
                                            <span class="slider round"></span>
                                        </label>
                                    </div>
                                    <div class="col-xs-4">
                                        <!-- <button class="btn btn-primary" @click.prevent="openTaskPanel(index)">Details</button> -->
                                        <button class="btn btn-primary" v-if="showDenyBtn(jobTask)"
                                                @click="openDenyTaskForm(jobTask)">
                                            Deny
                                        </button>
                                        <button class="btn btn-warning" v-if="showReopenBtn(jobTask)"
                                                @click="reopenTask(jobTask)" :disabled="disabled.reopen">
                      <span v-if="disabled.reopen">
                        <i class="fa fa-btn fa-spinner fa-spin"></i>
                      </span>
                                            Reopen
                                        </button>
                                        <button class="btn btn-danger" v-if="showDeleteBtn(jobTask)"
                      ...
                      <span v-if="disabled.deleteTask">
                        <i class="fa fa-btn fa-spinner fa-spin"></i>
                                    ...
                                    <div class="col-xs-4">
                                        <button class="btn btn-success" v-if="showPayForTaskBtn(jobTask)"
                                                @click.prevent="payForTask(jobTask)" :disabled="disabled.pay">
                      <span v-if="disabled.pay">
                        <i class="fa fa-btn fa-spinner fa-spin"></i>
                      </span>
                                            Pay
                                        </button>
                                        <button class="btn btn-success" v-if="showPayCashForTaskBtn(jobTask)"
                                                @click.prevent="paidWithCashTask(jobTask)" :disabled="disabled.payCash">
                      <span v-if="disabled.payCash">
                        <i class="fa fa-btn fa-spinner fa-spin"></i>
                      </span>
                                            Cash
                                        </button>
                                        <button class="btn btn-success" v-if="showFinishedBtn(jobTask)"
                      ...
                      <span v-if="disabled.finished">
                        <i class="fa fa-btn fa-spinner fa-spin"></i>
                                        ...
                                        <button class="btn btn-success" v-if="showApproveBtn(jobTask)"
                      ...
                      <span v-if="disabled.approve">
                        <i class="fa fa-btn fa-spinner fa-spin"></i>
                                ...
                                <div class="col-xs-12 hidden" :id="'task-subs-' + jobTask.id"
                                     v-if="isGeneral && !taskApproved" :key="3">
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th>Sub</th>
                                        ...
                                        <tr class="table" v-for="bid in jobTask.bid_contractor_job_tasks" :key="bid.id">
                                            ...
                                            <td>${{ bid.bid_price }}</td>
                                            <td>
                                                <button @click="preview(jobTask, bid.id)"
                                                        class="button btn btn-sm btn-info">Preview
                                                </button>
                                            </td>
                                            <td>
