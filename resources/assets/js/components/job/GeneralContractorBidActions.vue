<template>
    ...
    <div>
        <div class="flex flex-col w-full">
            <div class="text-white btn-red rounded p-3 mt-2 mb-2 text-center" style="font-weight: 700" v-show="subTaskWarning">PLEASE CHECK TASKS. SOME
                TASKS HAVE
                SUB PRICES HIGHER THAN CONTRACTOR PRICE
            </div>
            <div v-if="showPreApprovedActions">
                <div v-if="bid.job_tasks.length > 0" class="flex w-full justify-around">
                    <button class="btn btn-red flex-1" @click.prevent="openModal('confirmJobCancellation')"
                    ...
                    <button class="btn btn-green flex-1"
                ...
                <div v-else class="flex w-full justify-around">
                    ...
                    <button class="btn btn-blue flex-1 ml-6" name="addTaskToBid" id="addTaskToBid" @click="openAddTask"
                            v-if="bid.job_tasks.length > 0 || bid.job_tasks.length <= 0">
                        Add A Task
                    </button>
                </div>
