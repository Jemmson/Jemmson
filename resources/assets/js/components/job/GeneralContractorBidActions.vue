<template>
  ...
  <div>
    ...
    <div v-if="showPreApprovedActions" class="">
      <div class="btn-blue" v-if="bid.job_tasks.length <= 0">Please Add A Task
      </div>
      <div class="flex justify-between">
        <button class="btn-red" @click.prevent="openModal('confirmJobCancellation')" :disabled="disabled.cancelBid">
          <span v-if="disabled.cancelBid">
            <i class="fa fa-btn fa-spinner fa-spin"></i>
          </span>
          Cancel Job
        </button>
        <button class="btn-blue" name="addTaskToBid" id="addTaskToBid" @click="openAddTask" v-if="bid.job_tasks.length > 0 || bid.job_tasks.length <= 0">
          Add A Task
        </button>
        <button class="btn-green" v-if="bid.job_tasks.length > 0" @click="openModal('notifyCustomerOfFinishedBid')" :disabled="bid.job_tasks.length <= 0 || disabled.submitBid">
