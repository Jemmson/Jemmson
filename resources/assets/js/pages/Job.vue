<template>
  ...
  <div class="container">
    ...
    <v-card>
      ...
      <v-card-title class="w-break flex justify-content-around">

        <div v-if="editJobName">
          <div class="flex flex-col">
            <v-text-field
                id="jobName"
                label="JobName"
                v-model="bid.job_name"
...
<!--              </v-btn>-->
              ...
              <v-btn
                  ...
                  small
                  color="primary"
                  @click="editJobName = false">Cancel</v-btn>
              <v-btn
                  :loading="jobNameLoading"
                  class="mx-2"
                  small
                  color="primary"
                  @click="saveJobName()">Save
              </v-btn>
        ...
        <div v-else>{{ bid.job_name }}</div>
        <v-icon
            class="ml-2"
            color="red"
            @click="editJobName = !editJobName"
        >mdi-file-edit
        </v-icon>
...
export default {
  ...
  methods: {
    ...
    async saveJobName() {
      this.jobNameLoading = true;
      const {data} = await axios.post('/saveJobName', {
        jobId: this.bid.id,
        jobName: this.bid.job_name,
      })

      if (data.error) {

      } else {
        this.bid.job_name = data.jobName;
      }
      this.jobNameLoading = false;
