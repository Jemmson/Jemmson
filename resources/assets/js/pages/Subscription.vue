      ...
      <v-simple-table>
        <template>
          ...
          <tr v-for="invoice in invoices" :key="invoice.id">
            <td>{{ invoice.date }}</td>
            <td>{{ invoice.total }}</td>
            <td><a :href="invoice.stripeInvoiceUrl" target="_blank">Download</a></td>
            <td><a :href="invoice.stripeInvoicePdf" target="_blank">View</a></td>
          </tr>
          </tbody>
        </template>
      </v-simple-table>
...
export default {
  ...
  methods: {
    ...
    async showInvoices() {
      this.overlay = true
      const {data} = await axios.get('/subscription/getInvoices')
      if (data.error) {
