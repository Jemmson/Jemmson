  ...
  <v-container>
    ...
    >
      <v-card>
        ...
        <v-card-subtitle
            class="error_widget error--text"
            v-if="fileLoadingError"
        >There was an error loading your picture. Please try again.</v-card-subtitle>
...
export default {
  ...
  methods: {
    ...
    isGravitarImage(){
      let image = this.photoUrl.split('/');
      return this.user && image[2] === 'www.gravatar.com'
    },
    ...
    //   let formData = new FormData();
    //   formData.append('profilePhoto', this.profilePhoto);
    //   const {data} = await axios.post('/user/profileImage',  formData,
    //       {
    //         headers: {
    //           'Content-Type': 'multipart/form-data'
    //         }
